...
function onload(saved_data)
    ...
    myDeck = getObjectFromGUID("21f045")
    tracker = getObjectFromGUID("a1873a")
    tablet = getObjectFromGUID("420546")
    reveal = getObjectFromGUID("61fe0d")
    createButtons()
    createCheckbox()
    createTextbox()
    createReveal()
...
function click_reveal()
    ...
    cloneCardFromDeck(state.currentscheme, "b_reveal")
...
function set_startingschemes(schemes)
    ...
    for i, scheme in ipairs(state.currentoptions) do
        log(scheme)
        cloneCardFromDeck(scheme, "b_option"..i)
    end
...
function click_scheme(buttonIndex)
    ...
    cloneCardFromDeck(new_scheme, "current_blue")
...
function updateScore(value)
    ...
    for i=1,value,1 do tracker.call("add_subtract", {obj, "Blue", false}) end
...
function cloneCardFromDeck(cardName, tag)  
    ...
    if tag == "b_reveal" then target = reveal end
